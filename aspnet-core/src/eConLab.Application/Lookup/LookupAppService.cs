...
namespace eConLab.Lookup
{
       ...
       ILookupAppService
    {
        ...
        public async Task<List<DropdownListDto>> RequestsStatus()
        {

            return Enum.RequestStatus.GetValues(typeof(RequestStatus))
                          .Cast<RequestStatus>().Select(mod => new DropdownListDto
                          {
                              Name = mod.ToString(),
                              Id = (long)mod,
                          }).ToList();
        ...
        public async Task<List<DropdownListDto>> ProjectStatus()
        {

            return Enum.ProjectStatus.GetValues(typeof(ProjectStatus))
                          .Cast<ProjectStatus>().Select(mod => new DropdownListDto
                          {
                              Name = mod.ToString(),
                              Id = (long)mod,
                          }).ToList();
